Public Class frmNewTermTimetable1

    Private Sub frmNewTermTimetable_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
...
#Region "Page One"
    ...
    Public Sub btnToPage2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnToPage2.Click
        ...
        If bolBackPressed = False Then
            ...
            ReDim tsuTermTSU(intYears + 1, intTerms + 1)
        ...
        ElseIf bolBackPressed = True Then
            ...
            If intYears > CInt(txtYears.Text) Then
                ' Create a copy array
                Dim tsuCopy(intYears, intTerms) As TermSetUp

                ' Copy the current setup array
                Array.Copy(tsuTermTSU, tsuCopy, tsuTermTSU.Length)
                ...
                ReDim tsuTermTSU(intYears, intTerms)

                ' Populate the array
                For intCurrentYearEntry = 1 To intYears
                    For intCurrentTermEntry = 1 To intTerms
                        tsuTermTSU(intCurrentYearEntry, intCurrentTermEntry) = tsuCopy(intCurrentYearEntry, intCurrentTermEntry)
                    Next
                Next
            ElseIf intYears < CInt(txtYears.Text) Then
                ...
                ' Resize the term set up control array
                ReDim tsuTermTSU(CInt(txtYears.Text), intTerms)
            ...
            If intTerms > CInt(txtTerms.Text) Then
                ...
                For intCurrentYearEntry = 1 To intYears
                    For intCurrentTermEntry = CInt(txtTerms.Text) + 1 To intTerms
                        ' Delete the control
                        tsuTermTSU(intCurrentYearEntry, intCurrentTermEntry).Dispose()
                    Next
                Next
                ...
                ' Resize the term set up control array
                ReDim Preserve tsuTermTSU(intYears, intTerms)
            ElseIf intTerms < CInt(txtTerms.Text) Then
                ' Resize the term set up control array
                ReDim Preserve tsuTermTSU(intYears, CInt(txtTerms.Text))
...
#Region "Page Two"
    ...
    Private Sub btnCreate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        AddThreeFields("Timetable", "TimetableName", strTimetableName, "UserID", strUserInfo(intUserInfo.ID), "System", intTimetableSystem.TermSystem)
        ...
        For intCurrentYear As Integer = 1 To CInt(txtYears.Text.Trim)
            ...
            For intCurrentTerm As Integer = 1 To CInt(txtTerms.Text.Trim)
                With tsuTermTSU(intCurrentYear, intCurrentTerm)
                ...
                ' Create a new term
                AddSevenFields("Term", "Number", intCurrentTerm, "StartDate", strStartDate, "FinishDate", strFinishDate, "Weeks", tsuTermTSU(intCurrentYear, intCurrentTerm).txtLength.Text.Trim, "YearID", intTimetable(intEnumTimetable.YearID), "WeekStart", 1, "WeekEnd", tsuTermTSU(intCurrentYear, intCurrentTerm).txtLength.Text.Trim)
            Next
        Next
