Public Class frmNewTermTimetable
...
#Region "Page One"
    ...
    Public Sub btnToPage2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnToPage2.Click
        ...
        If bolBackPressed = False Then
            ...
            ReDim twsTerm(intYears, intTerms)
        ...
        ElseIf bolBackPressed = True Then
            ...
            If intYears > CInt(txtYears.Text) Then
                ' Create a copy array
                Dim twsCopy(intYears, intTerms) As TermAndWeekSetup

                ' Copy the current setup array
                Array.Copy(twsTerm, twsCopy, twsTerm.Length)
                ...
                For intCurrentYearEntry = 1 To intYears
                    For intCurrentTermEntry = 1 To intTerms
                        twsTerm(intCurrentYearEntry, intCurrentTermEntry) = twsCopy(intCurrentYearEntry, intCurrentTermEntry)
                    Next
                Next
            ElseIf intYears < CInt(txtYears.Text) Then
                ' Create a copy array
                Dim twsCopy(CInt(txtYears.Text), intTerms) As TermAndWeekSetup

                ' Copy the current setup array
                Array.Copy(twsTerm, twsCopy, twsTerm.Length)

                ' Resize the term set up control array
                ReDim twsTerm(CInt(txtYears.Text), intTerms)
            ...
            If intTerms > CInt(txtTerms.Text) Then
                ...
                For intCurrentYearEntry = 1 To intYears
                    For intCurrentTermEntry = CInt(txtTerms.Text) + 1 To intTerms
                        ' Delete the control
                        twsTerm(intCurrentYearEntry, intCurrentTermEntry).Dispose()
                    Next
                Next
                ...
                ' Resize the term set up control array
                ReDim Preserve twsTerm(intYears, intTerms)

            ElseIf intTerms < CInt(txtTerms.Text) Then
                ' Resize the term set up control array
                ReDim Preserve twsTerm(intYears, CInt(txtTerms.Text))
    ...
    Private Sub btnCreate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        For intCurrentYear As Integer = 1 To CInt(txtYears.Text.Trim)
            ...
            For intCurrentTerm As Integer = 1 To CInt(txtTerms.Text.Trim)
                With twsTerm(intCurrentYear, intCurrentTerm)
                ...
                ' Create a new term
                AddSevenFields("Term", "Number", intCurrentTerm, "StartDate", strStartDate, "FinishDate", strFinishDate, "Weeks", twsTerm(intCurrentYear, intCurrentTerm).txtLength.Text.Trim, "YearID", intTimetable(intEnumTimetable.YearID), "WeekStart", 1, "WeekEnd", twsTerm(intCurrentYear, intCurrentTerm).txtLength.Text.Trim)
            Next
        Next
